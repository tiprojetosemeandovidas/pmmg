...
function hasOnlyKeys(value, allowed) {
  return value && typeof value === 'object' && Object.keys(value).every(key => allowed.includes(key));
}
...
function validateEdital(data) {
  ...
  else data.etapas.forEach((stage, index) => { if (!hasOnlyKeys(stage, ['nome', 'tipo', 'detalhes']) || typeof stage.nome !== 'string' || typeof stage.tipo !== 'string' || (stage.detalhes !== null && typeof stage.detalhes !== 'string')) errors.push(`Etapa ${index + 1} inválida.`); });
  if (!Array.isArray(data.disciplinas)) errors.push('disciplinas deve ser uma lista.');
  else data.disciplinas.forEach((subject, index) => {
    if (!hasOnlyKeys(subject, ['nome', 'quantidade_questoes', 'peso', 'topicos'])) errors.push(`Disciplina ${index + 1} possui campos não permitidos.`);
  ...
  if (data.taf !== null) {
    if (!hasOnlyKeys(data.taf, ['existe', 'testes']) || typeof data.taf.existe !== 'boolean' || !Array.isArray(data.taf.testes)) errors.push('taf deve ser null ou uma estrutura válida.');
    else {
      data.taf.testes.forEach((physicalTest, index) => { if (!hasOnlyKeys(physicalTest, ['tipo', 'requisito', 'unidade']) || typeof physicalTest.tipo !== 'string' || (physicalTest.requisito !== null && typeof physicalTest.requisito !== 'string') || (physicalTest.unidade !== null && typeof physicalTest.unidade !== 'string')) errors.push(`Teste físico ${index + 1} inválido.`); });
      if (!data.taf.existe && data.taf.testes.length) errors.push('taf não pode listar testes quando existe é false.');
    }
  }
  ...
  if (typeof data.confianca_geral === 'number' && data.confianca_geral < 0.5 && Array.isArray(data.alertas_revisao) && !data.alertas_revisao.length) errors.push('Extrações de baixa confiança devem explicar o motivo em alertas_revisao.');
  if (validDate(data.data_inscricao_inicio) && validDate(data.data_inscricao_fim) && data.data_inscricao_inicio && data.data_inscricao_fim && data.data_inscricao_inicio > data.data_inscricao_fim) errors.push('data_inscricao_inicio não pode ser posterior a data_inscricao_fim.');
