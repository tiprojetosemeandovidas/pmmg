...
async function parse(response) {
  ...
  let data = null;
  if (text) {
    try { data = JSON.parse(text); } catch (_) { data = { message: text.slice(0, 300) }; }
  }
  if (!response.ok) {
...
async function uploadPdf(path, bytes) {
  const { url, serviceKey } = config();
  const response = await fetch(`${url}/storage/v1/object/editais-private/${path}`, {
    method: 'POST',
    headers: { apikey: serviceKey, Authorization: `Bearer ${serviceKey}`, 'Content-Type': 'application/pdf', 'x-upsert': 'false' },
    body: bytes
  });
  return parse(response);
}
...
module.exports = { authenticate, authenticateReviewer, createSignedUpload, downloadPdf, removePdf, rest, uploadPdf };
