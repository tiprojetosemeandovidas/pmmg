...
async function startDiagnosticSession(){if(!currentAccessToken)return null;const response=await fetch('/api/diagnostics',{method:'POST',headers:{Authorization:`Bearer ${currentAccessToken}`,'Content-Type':'application/json'},body:JSON.stringify({questionCount:20})});if(!response.ok)throw new Error('diagnostic_failed');const payload=await response.json();return payload.data.id}
...
document.querySelector('#diagnosisNext').addEventListener('click',async()=>{if(diagnosisStep<3){showDiagnosisStep(diagnosisStep+1);return}try{currentDiagnosticSessionId=await startDiagnosticSession()}catch{notify('Diagnóstico indisponível','Entre novamente e tente iniciar o diagnóstico.');return}diagnosisModal.classList.remove('open');enterDashboard('questoes');const pool=currentAccessToken?questions.filter(q=>q.kind==='database'):questions.filter(q=>Number.isInteger(q.answer));filteredQuestions=shuffled(pool).slice(0,20);questionIndex=0;answered={};renderQuestion();notify('Diagnóstico iniciado','Responda às questões para calcular seu nível e gerar o plano.')});
