...
function DashboardContent() {
  ...
  return (
    <div className="next-content">
      ...
      <section className="weekly-loop"><div><span className="weekly-kicker">DESAFIO DA SEMANA</span><h3>{state.diagnostic.active ? "Calibre sua rota em 10 questões" : "Feche a semana e recalibre"}</h3><p>{state.diagnostic.active ? "Cada resposta recalcula domínio, confiança e prioridade." : `Diagnóstico concluído. ${state.reviewQueue.length} item(ns) aguardam revisão.`}</p></div><div className="weekly-progress"><b>{state.diagnostic.answered}/{state.diagnostic.target}</b><i><span style={{ width: `${Math.min(100, (state.diagnostic.answered / state.diagnostic.target) * 100)}%` }} /></i>{state.diagnostic.active ? <button className="outline-button" type="button" onClick={() => router.push("/app/questoes")}>Continuar diagnóstico</button> : <button className="outline-button" type="button" onClick={completeWeeklyCheckin}>Fazer fechamento semanal</button>}</div></section>
